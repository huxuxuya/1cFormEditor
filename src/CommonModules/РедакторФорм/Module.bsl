...
Функция СоздатьКонтекстЭлемента(Форма, 
...
//	Свойства.Вставить("ПутьКДанным", "Объект.РасшифровкаПлатежа.ккОснованиеПлатежа");
//	Свойства.Вставить("Вид", ВидПоляФормы.ПолеВвода);
//	РедакторФорм.ДобавитьПолеНаФорму(ЭтаФорма, "ккРасшифровкаПлатежаОснованиеПлатежа", Свойства, Элементы.РасшифровкаПлатежа);
//
// Добавление элмента на форму
//	Свойства.Вставить("ПутьКДанным", "ВидДоговора");
//	РедакторФорм.ДобавитьПолеНаФорму(ЭтаФорма, "ВидДоговора", Свойства, Элементы.БыстрыеОтборы, Истина);
//
Функция ДобавитьПолеНаФорму(КонтекстЭлемента, ИмяПоля) Экспорт
...
// Пример:
//
//	РедакторФорм.ДобавитьПолеНаФормуРеквизитФормы(ЭтаФорма, "ккПравилаОплаты", Элементы.ГруппаСлева, Истина);
//
Функция ДобавитьПолеНаФормуРеквизитФормы(КонтекстЭлемента, ИмяРеквизита) Экспорт
...
//
Функция ДобавитьГруппуНаФорму(КонтекстЭлемента, ИмяГруппы) Экспорт
...
//
Функция ДобавитьГруппуСКолонкамиЛевоИПраво(КонтекстЭлемента, ИмяОсновнойГруппы) Экспорт
	...
	КонтекстНовогоЭлемента = СоздатьКонтекстЭлемента(Форма, ГруппаФормы, , СвойстваВложенных);
	
	ДобавитьГруппуНаФорму(КонтекстНовогоЭлемента, ИмяГруппыЛево);
	ДобавитьГруппуНаФорму(КонтекстНовогоЭлемента, ИмяГруппыПраво);

	Возврат ГруппаФормы;

КонецФункции

Функция ДобыавитьГруппуОбычную(КонтекстЭлемента, ИмяЭлемента) Экспорт
...
//	РедакторФорм.ДобавитьТаблицуНаФорму(ЭтаФорма, "ТаблицаМенеджеры", "Объект.ккМенеджеры", СтруктураКолонок, Элементы.ГруппаРодительБизнесРегион);
//
Функция ДобавитьТаблицуНаФорму(КонтекстЭлемента, ИмяПоля, ПутьКДанным, СтруктураКолонок = Неопределено) Экспорт
	...
	Если СтруктураКолонок <> Неопределено Тогда
		Для Каждого Элемент Из СтруктураКолонок Цикл
			...
			КонтекстНовогоЭлемента = СоздатьКонтекстЭлемента(Форма, ПолеФормы, , Параметры);
			
			ДобавитьПолеНаФорму(КонтекстНовогоЭлемента, Элемент.Ключ);
...
//	Родитель - Произвольный - Ссылка на родителя.
//	Свойства - Структура - Структура по которой заполняются свойства поля.
//	ЭтоПолеВвода - Булево - Определяет возможность ввода в добавляемое поле
//
// Возвращаемое значение:
//	ЭлементФормы - добавленный на форму, новый элемент формы.
...
//Пример:
//
// РедакторФорм.ДобавитьПолеТабличнойЧастиНаформу(ЭтаФорма, "Объект.Товары", "ккДатаОтгрузки", Элементы.Товары);
//
Функция ДобавитьПолеТабличнойЧастиНаформу(КонтекстЭлемента, ПутьТабличнойЧасти, ИмяКолонкиТабличнойЧасти) Экспорт
	...
	КонтекстНовогоЭлемента = СоздатьКонтекстЭлемента(Форма, Родитель, , СвойстваЭлемента);
	ПолеФормы = ДобавитьПолеНаФорму(КонтекстНовогоЭлемента, ИмяПоляТабличнойЧасти);
...
//	Родитель - Произвольный - Ссылка на родителя.
//
Функция ДобавитьКнопкуНаФорму(КонтекстЭлемента, ИмяКнопки, ВидКнопки, ИмяКоманды) Экспорт
...
//
Функция ДобавитьКомандуИГиперссылкуНаФорму(КонтекстЭлемента, ИмяКоманды, ПроцедураОбработки, ЗаголовокГиперссылки) Экспорт
	...
	КонтекстКнопки = СоздатьКонтекстЭлемента(Форма, Родитель, , Новый Структура("Вид", ВидКнопкиФормы.Гиперссылка));
	Кнопка = ДобавитьКнопкуНаФорму(КонтекстКнопки,
						ИмяКоманды,
...
//
Функция ДобавитьКомандуИКнопкуКоманднойПанели(КонтекстЭлемента, ИмяКоманды, ЗаголовокКнопки) Экспорт
...
//	РасположитьПередЭЛементом - ГруппаФормы; ТаблицаФормы; УправляемаяФорма - Элемент перед которым требуется разместить новый элемент.
//
Функция ДобавитьКомандуИКнопкуНаФорму(КонтекстЭлемента, ИмяКоманды, ЗаголовокКнопки) Экспорт
...
//
Функция ДобавитьРеквизитОбъектаНаФорму(КонтекстЭлемента, ИмяРеквизита) Экспорт
	...
	Если ТипЗнч(Форма.Объект[ИмяРеквизита]) = Тип("Булево") Тогда
		...
		Возврат ДобавитьПолеФлажкаНаФормуРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита);
	Иначе
		Возврат ДобавитьПолеНаФормуРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита);
	КонецЕсли;
...
//
Функция ДобавитьПолеНаФормуРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита) Экспорт
...
//
Функция ДобавитьПолеВШапкуФормыРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита) Экспорт
	...
	ПолеФормы = ДобавитьПолеНаФормуРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита);
...
//	ЭлементФормы - добавленный на форму, новый элемент формы.
//
Функция ДобавитьМногострочноеПолеРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита, Высота = 2) Экспорт
...
//
Функция ДобавитьПолеФлажкаНаФормуРеквизитОбъекта(КонтекстЭлемента, ИмяРеквизита) Экспорт
